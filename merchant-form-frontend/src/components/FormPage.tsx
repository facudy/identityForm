...
const FormPage: React.FC = () => {
    ...
    }, [validationError, messageApi]);

    const handleInputChange = useCallback((field: keyof MerchantFormData, value: string | string[]) => {
        setFormData(prev => ({ ...prev, [field]: value }));
    }, []);

    const showFileError = useCallback((errorMessage: string) => {
        setTimeout(() => {
            messageApi.error(errorMessage);
        }, 0);
    }, [messageApi]);

    const handleFileUpload = useCallback((field: FileUploadField, file: File, isMultiple = false) => {
        if (!file) return;

        // 检查文件大小 (5MB限制)
        if (file.size > 50 * 1024 * 1024) {
            showFileError('文件大小不能超过 50MB');
            return;
        }
    ...
    const removeFile = useCallback((field: FileUploadField, index?: number) => {
        if (index !== undefined) {
            setFiles(prev => ({
                ...prev,
    ...
    const validateForm = useCallback((): boolean => {
        ...
        if (formData.regionValues.length === 0) {
            setValidationError('请选择地区');
            return false;
        }
        if (!formData.detailAddress.trim()) {
            setValidationError('请输入详细地址');
            return false;
        }
        if (!formData.contactName.trim()) {
            setValidationError('请输入联系人姓名');
            return false;
        }
        if (!formData.phoneNumber.trim()) {
            setValidationError('请输入手机号码');
            return false;
        }
        if (!/^1[3-9]\d{9}$/.test(formData.phoneNumber)) {
            setValidationError('请输入正确的手机号码');
            return false;
        }
        if (!files.idCardFront) {
            setValidationError('请上传身份证人像面');
            return false;
        }
        if (!files.idCardBack) {
            setValidationError('请上传身份证国徽面');
            return false;
        }
        if (!files.businessLicense) {
            setValidationError('请上传营业执照');
            return false;
        }
        return true;
    }, [formData, files]);
    ...
    }, [formData, files, validateForm, messageApi]);
